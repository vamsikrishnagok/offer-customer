...
export default class Categories extends Component {
    ...
    componentDidMount() {
        fetch( myConfig.apiBackendUrl+"customer/categoryList/",{
            ...
            .then((result) => {
                    console.log(result)
                    this.setState({categories: result.data.map( (res,_) => {
                            return <CategoryCard name={res.name} id={res.id} icon={res.icons}/>
            ...
            <>
                <div className="col s12 m6">
                    ...
                    <div className="col s12 whiteBg z-depth-1 valign-wrapper whiteTextBox top-margin">
                        <div className="col s12 center-align greyText">Don't know the category? View all the <Link to="/offers" style={{color: "rgb(23, 43, 84)"}}><a
                            className="textLink greyText">offers</a></Link>
                        </div>
                    </div>
                </div>
            </>
